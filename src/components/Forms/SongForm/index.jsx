...
const SongForm = () => {
  ...
  // Handle song duration calculation
  const handleAudioDuration = (file) => {
  ...
  const handleInputState = (name, value) => {
    ...
    // Handle audio duration when song file is selected
    if (name === "song" && value instanceof File) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!error) {
      ...
      if (id === "new") {
        const res = await createSong(formData, dispatch);
        res && history.push("/songs");
      } else {
        const res = await updateSong(id, formData, dispatch);
        res && history.push("/songs");
      }
    } else {
